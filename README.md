...
This project is an exploration of a transformer neural network language model trained on Canadian parliamentary transcripts.
This is similar to what's being done over at OpenAI with the
[GPT](https://www.technologyreview.com/2020/07/20/1005454/openai-machine-learning-language-generator-gpt-3-nlp/)
models,
except on a smaller scale, with less intricacies ... so not quite the same.
But it is based on the same technology and the same principle of unsupervised learning.

The two main objectives of the project are:
allow for semantic (non-keyword) search of texts,
and generate texts in the style of parliamentary transcripts.

For the semantic search,
the objective is to create a model which can take in some natural text,
and output a vector in Euclidean space.
...
Before getting too deep into the technical details,
...
> Mr. Speaker, if the minister was asking for a better answer, he can answer the question.
